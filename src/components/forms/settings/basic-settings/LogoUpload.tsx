...
export default function LogoUpload({ value, setValue }: { 
  ...
  const handleFileSelect = async (file: File) => {
    ...
    if (!allowedTypes.includes(file.type) && !['png', 'jpg', 'jpeg', 'svg'].includes(fileExtension || '')) {
...
const removeLogo = async () => {
  ...
  try {
    ...
    const res = await fetch(
      `${apiUrl}/seller/settings/basic-settings/delete-logo.php`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (result.success) {
      setValue("");
      toast.success("Logo removed & deleted from server!");
    } else {
      ...
      >
        ...
        {uploading ? (
          ...
          <div className="relative group">
            <div className="w-48 h-48 rounded-full overflow-hidden bg-white p-6">
              <Image
                src={previewUrl}
                alt="Logo"
                width={160}
